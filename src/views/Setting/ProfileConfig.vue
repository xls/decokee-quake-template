        ...
        <div class="content" style="position: relative">
            <div class="dropdownMenu">
                <div class="el-container">
                    ...
                    <div class="handleMenu">
                        <el-button class="btn-folder" icon="Plus" plain size="small" type="primary" @click="addDropdownMenu"></el-button>
                        <el-button :disabled="!!(selectedIndex && dropdownMenu[selectedIndex].value === '1-0')" class="btn-folder" icon="Minus" plain size="small" type="primary" @click="deleteDropdownMenu"></el-button>
                    </div>
                </div>
            </div>
            ...
            <div class="dropdownMenu-right">
                <div v-if="selectedIndex !== null && checkedProfileConfigs.length === 1">
                    ...
                    <el-form size="small" style="padding: 20px">
                        <el-form-item :label="$t('settings.selectApplication')">
                            <el-select ref="selectApplicationDropdown" v-model="selectedApplication" :placeholder="$t('pleaseSelect')" size="small" filterable clearable @change="handleApplicationSelected">
                        ...
                        <el-form-item :label="$t('settings.selectDevice')">
                            <el-select ref="selectDeviceDropdown" v-model="selectedMonitorDevice" :placeholder="$t('pleaseSelect')" size="small" filterable @change="handleDeviceSelected">
...
export default {
    ...
    created() {
        ...
        ipcRenderer.on('subpage-newDeviceMenu', (event, data, row, col) => {
            // Handle data sent from the main window
    ...
    methods: {
        async getInstalledAppInfo() {
            ...
            for (let i = 0; i < currentInstalledApps.length; i++) {
                ...
                // Skip if appIdentifier already exists, or appLaunchPath ends with .lnk
                if (seen[appIdentifier] || appLaunchPath.endsWith('.lnk')) {
                    continue;
                }

                // If appIdentifier does not exist yet, record it
        ...
        onContextmenu(event, index, resourceId, label) {
            ...
            this.$contextmenu({
                ...
                x: event.x, // mouse position
                y: event.y,
                customClass: 'right-click-menu', // custom menu class
                zIndex: 1, // menu z-index
                minWidth: 140, // main menu min width
            });
            return false;
        },
        ...
        getDefaultResource() {
            ...
            // console.log("profileConfig  default config file", this.defaultResource);
        ...
        handleIsDefault(val) {
            ...
            console.log('profileConfig  default set successfully', result);
            this.getDefaultResource();
        },
        getAllResource() {
            this.allResource = window.resourcesManager.getAllResourceInfoByType(Constants.RESOURCE_TYPE_DEVICE_CONFIG);
            // console.log("profileConfig  full resource list", this.allResource);
        },
        getAddConfigInfo(configId, page, folder) {
            ...
            console.log('profileConfig  add key config', this.resourceId);
        },
        ...
        addDropdownMenu() {
            ...
            console.log('profileConfig  max config file', nextConfigId);
        ...
        deleteDropdownMenu() {
            ...
            console.log('profileConfig  includes', includesArr);
            const firstNumbersSet = new Set(includesArr.map(item => item.name.split(',')[0]));
            // Use filter to select elements whose first number in name is in the Set
            ...
            this.$confirm(this.$t('settings.deleteConfig'), '', {
                ...
                .then(() => {
                    ...
                    console.log('profileConfig  delete menu item', filteredMenu);
                ...
                .catch(err => {
                    console.log('profileConfig  delete menu item err: ', err);
                }).finally(() => {
