    ...
    async quitApp(restart = false) {
        ...
        if (this.windowManager.windowList) {
            this.windowManager.windowList.forEach(browserWindow => {
                // A window may have already been closed (win === null) or destroyed;
                // skip it instead of dereferencing a null/destroyed handle.
                const win = browserWindow && browserWindow.win;
                if (!win || win.isDestroyed()) return;
                console.log('MainApp: quitApp: destroying window: ', win.getTitle());
