...
import RemoteScreenWindow from '@/main/windows/RemoteScreenWindow';
...
let quakeScreenWindow = undefined;
...
class DeviceControlManager {
    constructor(mainAppManager) {
        ...
        quakeScreenWindow = new RemoteScreenWindow(mainAppManager);
            ...
            HIDDeviceManager.registerDeviceConnectionListener(
                ...
                connectionInfo => {
                    ...
                    if (connectionInfo.type === CONNECTION_TYPE_QMK) {
                        // The QUAKE display: show/hide its on-panel content window
                        // (the backlight itself is kept lit by HIDManager's keep-alive).
                        if (connectionInfo.product === QUAKE_PRODUCT_NAME) {
                            if (connectionInfo.connected) {
                                quakeScreenWindow.open();
                                // Forward the panel's touch and the raw report stream
                                // (knob, state, pongs, ...) to the on-panel debug display.
                                const quakeService = HIDDeviceManager.getQuakeService(connectionInfo.serialNumber);
                                if (quakeService) {
                                    quakeService.on('touch', points => quakeScreenWindow.pushTouch(points));
                                    quakeService.on('report', report => quakeScreenWindow.pushReport(report));
                                }
                            } else {
                                quakeScreenWindow.close();
                            }
                        }
    ...
    // --- QUAKE display controls (used by the Device settings dialog) --------

    /** List connected QUAKE displays with their cached firmware/state. */
    getQuakeDevices() {
        const list = [];
        deviceTypeMap.forEach((connectionType, serialNumber) => {
            if (connectionType !== CONNECTION_TYPE_QMK) return;
            const service = HIDDeviceManager?.getQuakeService(serialNumber);
            if (!service) return;
            const state = service.getState();
            list.push({
                serialNumber,
                label: `DecoKee QUAKE ${serialNumber.slice(-4)}`,
    ...
    /** Cached hardware state (version/brightness/mic/buzzer) for a QUAKE. */
    getQuakeState(serialNumber) {
        return HIDDeviceManager?.getQuakeService(serialNumber)?.getState() || null;
    }

    /** Re-query firmware/brightness/mic from the device and return the state. */
    async refreshQuakeState(serialNumber) {
        const service = HIDDeviceManager?.getQuakeService(serialNumber);
        return service ? service.refreshState() : null;
    }

    setQuakeBrightness(serialNumber, value) {
        HIDDeviceManager?.getQuakeService(serialNumber)?.setBrightness(value);
    }

    setQuakeMic(serialNumber, enabled) {
    ...
    enterQuakeDownloadMode(serialNumber) {
        HIDDeviceManager?.getQuakeService(serialNumber)?.enterDownloadMode();
    }
    ...
    async destroy() {
        ...
        if (quakeScreenWindow) {
            quakeScreenWindow.close();
        }
        quakeScreenWindow = undefined;
