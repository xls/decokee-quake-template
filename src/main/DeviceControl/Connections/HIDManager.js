...
import { QuakeDevice } from "../../../../modules/quake-device";
...
// Product name reported by the DecoKee QUAKE display over its VIA interface.
const QUAKE_PRODUCT_NAME = 'QUAKE';

let HidActiveDeviceMap = new Map();
let QMKActiveDeviceMap = new Map();
// serialNumber -> QmkHidService (open VIA connection + keep-alive) for QUAKE devices.
const QuakeServiceMap = new Map();
...
class HIDManager {
    ...
    checkForHidDevices() {
        ...
        try {
            ...
            console.log('Found new device:', deviceInfo.product, 'serialNumber:', serialNumber);
    ...
    _CheckForQMKDevice(devices) {
        ...
        if (!deviceInfo || deviceInfo.length === 0) {
            if (this.connectionListeners) {
                for (let serialNumber of QMKActiveDeviceMap.keys()) {
                    const product = QMKActiveDeviceMap.get(serialNumber)?.product;
                    this.connectionListeners.forEach((listener, tag) => {
                        listener({serialNumber: serialNumber, connected: false, type: 'QMK', product});
                    });
                }
            }
            for (let serialNumber of QuakeServiceMap.keys()) {
                this._stopQuakeService(serialNumber);
            }
        ...
        for (let device of deviceInfo) {
            ...
            console.log('Find new QMK device:', device.product, 'serialNumber:', device.serialNumber);

            // For the DecoKee QUAKE display: open its VIA interface and start the keep-alive
            // ping so the firmware watchdog keeps the screen powered on. Do this before
            // notifying listeners so they can reach the service via getQuakeService().
            if (device.product === QUAKE_PRODUCT_NAME && !QuakeServiceMap.has(device.serialNumber)) {
                this._startQuakeService(device);
            }

            if (this.connectionListeners) {
                this.connectionListeners.forEach((listener, tag) => {
                    listener({serialNumber: device.serialNumber, connected: true, type: 'QMK', product: device.product});
                });
            }
        }
    }

    _startQuakeService(device) {
        try {
            const service = new QuakeDevice({ path: device.path });
            QuakeServiceMap.set(device.serialNumber, service);
            // Turn the screen on, set a visible brightness, and start the watchdog ping so
            // the firmware keeps the panel backlight powered (it blanks without keep-alive).
            service.wake();
            // Populate the cached hardware state (firmware/brightness/mic) for the
            // settings UI; values arrive asynchronously as 0x55 state reports.
            service.refreshState()
                .then(state => console.log(`HIDManager: QUAKE ${device.serialNumber} firmware ${state.version}`))
                .catch(() => {});
            console.log(`HIDManager: QUAKE service started for ${device.serialNumber}; wake + keep-alive sent.`);
        } catch (e) {
            console.error('HIDManager: failed to start QUAKE service for ' + device.serialNumber + ': ' + e.message);
            QuakeServiceMap.delete(device.serialNumber);
        }
    }

    _stopQuakeService(serialNumber) {
        const service = QuakeServiceMap.get(serialNumber);
        if (!service) return;
        console.log('HIDManager: stopping QUAKE service for ' + serialNumber);
        service.close();
        QuakeServiceMap.delete(serialNumber);
    }

    getQuakeService(serialNumber) {
        return QuakeServiceMap.get(serialNumber);
    }
